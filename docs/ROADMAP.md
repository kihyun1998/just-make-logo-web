...
## Phase 7: Supabase 저장 + 멀티 에셋 관리

목표: **로고/에셋 설정을 서버에 저장** + 멀티 스크린샷 관리 + 일괄 내보내기
...
### Step 7-3. 에셋 프로젝트 저장 + 멀티 슬롯
- [ ] 에셋 설정 → Supabase 저장 (스펙 + 템플릿 + 오버라이드 전체)
- [ ] 멀티 스크린샷 탭 UI (같은 템플릿으로 여러 장 관리 — 탭별 스크린샷/텍스트만 다름)
- [ ] 에셋 프로젝트 목록 + 불러오기

### Step 7-4. 일괄 에셋 내보내기 (이전 Step 5-4)
- [ ] 플랫폼별 필수 에셋 체크리스트 UI
- [ ] 멀티 슬롯 에셋 일괄 렌더링 → ZIP 다운로드
- [ ] 폴더 구조: `android/`, `ios/` 분리

**산출물:** Phase 7 완성. 서버 저장 + 멀티 에셋 관리 + 일괄 내보내기
